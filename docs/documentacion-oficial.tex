...
% ─── Código fuente (listings, sin dependencia Python) ────────
\usepackage{listings}
\lstset{
...
% Definición de lenguaje PHP básico (keywords y colores)
\lstdefinelanguage{phpex}{
  language=PHP,
  morekeywords={declare,strict_types,readonly,match,fn,null,true,false},
  keywordstyle=\color{komorebi}\bfseries,
  commentstyle=\color{gray}\itshape,
  stringstyle=\color{komorebi-dark},
}
\usepackage{tcolorbox}
\tcbuselibrary{listings,skins,breakable}
\newtcblisting{phpcode}[2][]{
  listing only,
  listing options={language=phpex,basicstyle=\small\ttfamily,numbers=left,
    numberstyle=\tiny\color{gray},breaklines=true,showstringspaces=false,
    keywordstyle=\color{komorebi}\bfseries,commentstyle=\color{gray}\itshape,
    stringstyle=\color{komorebi-dark}},
  title=#2,
  colback=code-bg,
  colframe=komorebi,
...
\newtcblisting{sqlcode}[2][]{
  listing only,
  listing options={language=SQL,basicstyle=\small\ttfamily,numbers=left,
    numberstyle=\tiny\color{gray},breaklines=true,showstringspaces=false,
...
\newtcblisting{bashcode}[2][]{
  listing only,
  listing options={language=bash,basicstyle=\small\ttfamily,numbers=left,
    numberstyle=\tiny\color{gray},breaklines=true,showstringspaces=false,
    keywordstyle=\color{gray!60!black}\bfseries},
  title=#2,
  colback=code-bg,
  colframe=gray,
  fonttitle=\bfseries,
...
\usepackage{amsmath}% requerido por cleveref
...
% ─── PORTADA (plantilla oficial IES Ágora) ──────────────────
\IfFileExists{portada-oficial.pdf}{
  \includepdf[pages=1]{portada-oficial.pdf}
}{
  % Portada temporal mientras se añade portada-oficial.pdf
  \begin{titlepage}
    \centering\vspace*{4cm}
    {\large\textbf{[PORTADA OFICIAL IES ÁGORA]}}\\[1cm]
    {\normalsize Guardar la portada oficial como \texttt{docs/portada-oficial.pdf}}
  \end{titlepage}
}
  ...
  \begin{sequencediagram}
    ...
    \begin{call}{C}{POST /api/v1/tokens (email, password)}{TC}{}
      \begin{call}{TC}{create(\$userId, \$name, \$abilities)}{TS}{}
        \begin{call}{TS}{\texttt{random\_bytes(32)} $\to$ plain token}{TS}{}
        \end{call}
...
% ============================================================
%  CAPÍTULO 3 — ALTERNATIVAS CONSIDERADAS
% ============================================================
\chapter{Alternativas Consideradas}
\label{ch:alternativas}
% TODO: contenido Fase 3.B
...
% ─── APÉNDICE E — MANUAL TÉCNICO ─────────────────────────────
\chapter{Manual Técnico}
\label{ch:manual-tecnico}
% TODO: contenido Fase 3.D
