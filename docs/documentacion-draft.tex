...
% \usepackage{svg}         % DRAFT: deshabilitado (requiere Inkscape)

% ─── Código fuente (DRAFT: listings en lugar de minted) ─────
\usepackage{listings}
\lstset{
  ...
  breaklines=true,
  frame=l, framerule=2pt, framesep=8pt,
  showstringspaces=false,
  keywordstyle=\color{komorebi-dark}\bfseries,
  commentstyle=\color{gray}\itshape,
  stringstyle=\color{komorebi},
  tabsize=4,
  alsoletter={_},
  texcl=false,
  escapeinside={}
}
% Prevenir que listings active > como carácter especial (PHP usa ?> y =>, ->)
% Esto es necesario para que TikZ siga funcionando con [->,dashed] etc.
% Lenguajes seguros sin active chars que rompan TikZ
\lstdefinelanguage{PHPsafe}{%
  keywords={declare,strict_types,namespace,use,class,interface,readonly,%
    final,abstract,public,protected,private,static,function,return,new,null,%
    true,false,if,else,elseif,foreach,for,while,do,switch,case,break,continue,%
    throw,try,catch,finally,match,fn,yield,echo,print,require,include,%
    require_once,include_once,void,string,int,float,bool,array,object,mixed,%
    never,self,parent,Result,Container,Queue,Logger,Flash,View,Raw,Env},
  sensitive=true,
  morestring=[b]",
  morestring=[b]',
  morecomment=[l]{//},
...
\lstdefinelanguage{SQLsafe}{%
  keywords={SELECT,INSERT,UPDATE,DELETE,FROM,WHERE,JOIN,LEFT,RIGHT,INNER,%
    OUTER,ON,AS,AND,OR,NOT,IN,IS,NULL,LIKE,BETWEEN,ORDER,GROUP,BY,HAVING,%
    LIMIT,OFFSET,CREATE,TABLE,INDEX,DROP,ALTER,ADD,COLUMN,PRIMARY,KEY,%
    FOREIGN,REFERENCES,UNIQUE,DEFAULT,CASCADE,SET,VALUES,INTO,BEGIN,%
    COMMIT,ROLLBACK,TRANSACTION,TRIGGER,PROCEDURE,FUNCTION,VIEW,%
    CONSTRAINT,CHECK},
  sensitive=false,
  morestring=[b]',
  morecomment=[l]{--},
...
\lstdefinelanguage{BASHsafe}{%
  keywords={if,then,else,elif,fi,for,do,done,while,until,case,esac,%
    function,return,exit,echo,read,export,local,source,set,unset},
  sensitive=true,
  morestring=[b]",
  morestring=[b]',
  morecomment=[l]{\#},
...
\lstnewenvironment{phpcode}[1][]{%
  \lstset{language=PHPsafe,alsoletter={_},basicstyle=\small\ttfamily,%
    backgroundcolor=\color{code-bg},frame=single,rulecolor=\color{komorebi},%
    breaklines=true,#1}%
}{}%
\lstnewenvironment{sqlcode}[2][]{%
  \lstset{language=SQLsafe,basicstyle=\small\ttfamily,%
    backgroundcolor=\color{code-bg},frame=single,rulecolor=\color{komorebi-dark},%
    caption={#2},captionpos=t,breaklines=true,#1}%
}{}%
\lstnewenvironment{bashcode}[2][]{%
  \lstset{language=BASHsafe,basicstyle=\small\ttfamily,%
    backgroundcolor=\color{code-bg},frame=single,rulecolor=\color{gray},%
    caption={#2},captionpos=t,breaklines=true,#1}%
}{}%
...
\begin{titlepage}
  ...
  % Logo del café (DRAFT: SVG deshabilitado)
  {\fontsize{28}{32}\selectfont\color{komorebi}\bfseries Komorebi Café}\\[1.5cm]
  ...
  % \includegraphics[width=2.5cm]{images/logos/komorebi-logo-icon}\\[0.5cm] % DRAFT: sin PNG
...
más de 250~rutas, 53~servicios de negocio, 29~repositorios y aproximadamente
80~endpoints de \gls{api} \gls{rest}.
...
and approximately 80 REST API endpoints.
...
\begin{longtable}{@{}lll@{}}
  ...
  Tests unitarios    & PHPUnit              & 11.x, tests paralelos\\
  ...
  \begin{sequencediagram}
    ...
    \begin{call}{U}{POST /api/v1/reservations -- Idempotency-Key: UUID v4}{IM}{}
      ...
      \begin{sdblock}{Cache HIT}{}
        \mess{IM}{Response cacheada: 200 o 201}{U}
      \end{sdblock}
      \begin{sdblock}{Cache MISS}{}
        ...
        \end{call}
        \mess{IM}{201 Created: invoice-pdf-url nulo}{U}
      \end{sdblock}
    \end{call}
  \end{sequencediagram}
...
\begin{phpcode}[caption={Pasar JSON seguro a Alpine.js}]
...
\begin{phpcode}[caption={Ejemplo de uso del Result pattern}]
<?php declare(strict_types=1);
...
\begin{phpcode}[caption={Consumo del Result en el controlador}]
<?php declare(strict_types=1);
...
\begin{longtable}{@{}llll@{}}
  ...
  % ── API Operaciones ──
  POST & \texttt{/api/v1/ops/reservations/\{id\}/checkin}& Api\textbackslash V1\textbackslash Ops\textbackslash ReceptionApiController & apiAuth, role(reception), csrf\\
  POST & \texttt{/api/v1/ops/orders/\{id\}/complete}& Api\textbackslash V1\textbackslash Ops\textbackslash KitchenApiController & apiAuth, role(kitchen), csrf\\
\end{longtable}
\end{landscape}
...
\begin{phpcode}[caption={Estructura de test unitario}]
<?php declare(strict_types=1);
...
% ============================================================
% \backmatter  % DRAFT: solo disponible en book class
...
\begin{longtable}{@{}lllp{5cm}@{}}
  ...
  GET  & \texttt{/ops/reservations}          & \dag & Reservas del día\\
  POST & \texttt{/ops/reservations/\{id\}/checkin}   & \dag & Check-in\\
  POST & \texttt{/ops/reservations/\{id\}/checkout}  & \dag & Check-out\\
  POST & \texttt{/ops/reservations/\{id\}/items}     & \dag & Añadir ítems consumidos\\
  POST & \texttt{/ops/reservations/\{id\}/payments}  & \dag & Registrar pago\\
  % ── Operaciones: Cocina ──
  GET  & \texttt{/ops/orders}                & \dag & Órdenes activas\\
  POST & \texttt{/ops/orders/\{id\}/complete}& \dag & Marcar orden como completada\\
  % ── Supervisor ──
...
\printunsrtglossary[type=abbreviations,title=Acrónimos]
\printunsrtglossary[type=main,title=Términos]
