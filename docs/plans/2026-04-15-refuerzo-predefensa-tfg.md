...
**Estado:** ✅ F1–F6 completadas — verificado y cerrado (16 abril 2026)
...
- [x] F5.1 — Ejecutar `make phpstan` con `reportUnmatchedIgnoredErrors: true` → ver qué de las 64 entradas son ya innecesarias
- [x] F5.2 — Eliminar entradas del baseline que ya no matchean código real
- [x] F5.3 — Corregir las entradas restantes: type shape mismatches en DTOs, propiedades write-only
- [x] F5.4 — Vaciar `phpstan-baseline.neon` completamente
- [x] F5.5 — Ejecutar `make phpstan` → `[OK] No errors`
- [x] F5.6 — ~~Ejecutar `make psalm`~~ — **N/A**: Psalm eliminado del ecosistema (solapamiento con PHPStan 5). Decisión documentada en `CONTRIBUTING.md` y plan `infra-calidad-integral.md` F3.
- [x] F5.7 — ~~Vaciar `psalm-baseline.xml`~~ — **N/A**: archivo eliminado junto con `bin/psalm.bat`.
- [x] F5.8 — ~~Ejecutar `make psalm` sin errores~~ — **N/A**: ver F5.6.
- [x] F5.9 — `make cs-check` → **0 de 736 archivos con violaciones PSR-12**, EXIT:0 ✅ (462 archivos normalizados automáticamente con `make cs-fix`)
...
- [x] F6.1 — `make ci` = phpstan ✅ + cs-check ✅ + tests ✅ (0 errors, 0 failures, 0 risky) → pipeline verde
- [x] F6.2 — N/A: `make e2e-a11y` omitido (fuera de alcance pre-defensa inmediata)
- [x] F6.3 — Métricas finales documentadas (16 abril 2026):
  - PHPStan nivel 5: **0 errores**, baseline vacío (`ignoreErrors: []`) ✅
  - PHP CS Fixer: **0 violaciones PSR-12** en 736 archivos ✅
  - Tests: **0 errors, 0 failures, 0 risky** (Unit + Integration) ✅
  - Psalm: **eliminado** (decisión documentada en CONTRIBUTING.md) ✅
  - `make ci` (phpstan + test + cs-check): **verde** ✅
- [x] F6.4 — Argumentario listo: PHPStan nivel 5 sin baseline, PSR-12 en todo el codebase (462 archivos normalizados), suite de tests en verde con 0 fallos
